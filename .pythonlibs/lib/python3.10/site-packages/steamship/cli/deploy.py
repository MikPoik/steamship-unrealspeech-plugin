...
def update_config_template(manifest: Manifest):
  ...
  if manifest.configTemplate != config_parameters:
    if len(config_parameters) > 0:
      click.secho("Config parameters changed; updating steamship.json.",
                  fg="cyan")
      ...
      click.secho("Config parameters removed; updating steamship.json.",
                  fg="cyan")
...
def get_archive_path(manifest: Manifest) -> Path:
  return Path(
      "."
  ) / "build" / "archives" / f"{manifest.handle}_v{manifest.version}.zip"
...
class DeployableDeployer(ABC):
  ...
  @abstractmethod
  def _create_version(self, client: Steamship, manifest: Manifest,
                      thing_id: str):
    pass
  ...
  def create_or_fetch_deployable(self, client: Steamship, user: User,
                                 manifest: Manifest):
  ...
  def create_version(self, client: Steamship, manifest: Manifest,
                     thing_id: str):
    version = None
    ...
    while version is None:
      click.echo(
          f"Deploying version {manifest.version} of [{manifest.handle}]... ",
          nl=False)
      try:
        with ship_spinner():
        ...
        if "The object you are trying to create already exists." in e.message:
          ...
          click.secho(f"\nUnable to deploy {self.deployable_type()} version.",
                      fg="red")
  ...
  def ask_for_new_version_handle(self,
                                 manifest: Manifest,
    ...
    try:
      default_new = str(
          VersionInfo.parse(manifest.version).next_version("prerelease"))
    except ValueError:
      pass
